...
//<editor-fold desc="Integral Validation" defaultstate="collapsed">
    public static function isInt($aug) {
        $txt = antiWhiteSpace($aug);    //remove whitspaces, for they have an ASCII equivalent of 32
        if (!isset($txt)) {
            return NULL;
...
abstract class Dates{
    ...
    public static function getDays($month)
    {
        ...
        if(Dates::isLeapYear($today->format('Y'))){  $days  = array(31,29,31,30,31,30,31,31,30,31,30,31);    }
        else {  $days  = array(31,28,31,30,31,30,31,31,30,31,30,31);    }
    return  $days[$month-1];
    }
}
    ...
    public static function ExceptionLog($message, $sql = "") {
        ...
        if (!empty($sql)) {
            ...
            if (!$log->write($message)) {
                throw new Exception("&FilledVerySmallSquare;&nbsp;&nbsp;$message<br/>&nbsp;&nbsp;&FilledVerySmallSquare;&nbsp;&nbsp;could not write log file &nbsp;&nbsp;");
            }
        } catch (Exception $ex) {
...
abstract class File{
    ...
    public static function uploadIMG($fileinput, $path, $file, $square, $sizelimit = null) {
        try {
            ...
            if(!isset($_FILES[$fileinput]['error']) || is_array($_FILES[$fileinput]['error'])){                throw new RuntimeException('Invalid parameters.');}
            ...
            // test filesize
            if(isset($sizelimit)){if($_FILES['imgPath']['size'] > $sizelimit){throw new RuntimeException('Exceeded filesize limit.');}}
            $finfo = new finfo(FILEINFO_MIME_TYPE);
            if(false === $ext = array_search($finfo->file($_FILES['imgPath']['tmp_name']), array('jpg' => 'image/jpeg','png' => 'image/png','bmp' => 'image/bmp','gif' => 'image/gif'), true)){throw new RuntimeException('Invalid Image Format.');}
...
//            if(!move_uploaded_file($_FILES[$fileinput]['tmp_name'],sprintf("$path/%s.%s",sha1_file($_FILES[$fileinput]['tmp_name']),$ext))){                throw new RuntimeException('Failed to move uploaded file.');}
            if(!move_uploaded_file($_FILES[$fileinput]['tmp_name'],sprintf("%s/%s.%s",$path,$file,$ext))){                throw new RuntimeException('Failed to move uploaded file.');}
    ...
    public static function scaleIMG($path, $file, $dim = NULL) {
        ...
        if(isset($dim)){
            if(is_array($dim)){$im2 = imagecrop($im, ['x' => 0, 'y' => 0, 'width' => $dim['x'], 'height' => $dim['y']]);}
            else {$im2 = imagecrop($im, ['x' => 0, 'y' => 0, 'width' => $dim, 'height' => $dim]);}
        }
        else {
